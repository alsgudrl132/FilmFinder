			...
			<div class="header-content">
				...
				<div class="basic-info">
					...
					<p
						...
						v-if="receivedData.original_title !== receivedData.title"
					>
						{{ receivedData.original_title }}
					</p>
					<div class="meta-info">
						<span class="rating">
				...
				<section class="additional-info">
					<div class="info-grid">
						<div class="info-item">
							<h3>상태</h3>
							<p>
								{{ receivedData.status === 'Released' ? '개봉' : '개봉 예정' }}
							</p>
						</div>
						<div class="info-item">
							<h3>개봉일</h3>
							<p>{{ receivedData.release_date }}</p>
						</div>
						<div class="info-item">
							<h3>제작비</h3>
...
export default {
	...
	async created() {
		...
		try {
			const response = await getMovieTvDetail('movie', '한국', routeId)
			this.receivedData = response.data
			const similarResponse = await getSimilarMovieTv('movie', '한국', routeId)
			this.similarData = similarResponse.data.results
			const casterResponse = await getCaster('movie', '한국', routeId)
